...
I use a :large_blue_diamond: emoji below to indicate something you should do. 

:large_blue_diamond: Login to your account on the Storrs HPC cluster:

    ssh hpc
    
:large_blue_diamond: Type the following to reserve an interactive session:
...
or you can create an alias by opening the _.bashrc_ file in your home directory:
...
:large_blue_diamond: Create a directory named _iqlab_ to serve as a playground for this week's lab:

    mkdir ~/iqlab

## Create a text file in which to save answers

:large_blue_diamond: Copy the text below into a text file to record you answers to questions posed in this lab (labeled with the :thinking: emoji).
...
:large_blue_diamond: Download the latest version (3.0.1) of IQ-TREE for "64-bit Linux Intel" (I got the link from the [downloads web page](http://www.iqtree.org/#download)):
...
:large_blue_diamond: How do you extract a `tar` file? Using the `tar` command of course!
...
:large_blue_diamond: Copy the `iqtree3` file (which is the actual program) to your own `~/bin` directory so that it will be accessible from anywhere. 

    cd ~/iqlab
    cd iqtree-3.0.1-Linux-intel/bin
    mv iqtree3 ~/bin

:large_blue_diamond: You can now delete the tar file and the directory that was created when we extracted it:
...
The tutorial below is a shortened version of [the one written by Bui Minh](http://www.iqtree.org/workshop/molevol2023) (the primary developer of IQ-TREE) for the 2023 [Woods Hole Workshop in Molecular Evolution](https://molevolworkshop.github.io). 

## Download the data

:large_blue_diamond: Download the sequence data file (`turtle.fa`) and partition file (`turtle.nex`) that we will be analyzing today:
...
:large_blue_diamond: Create a directory inside `iqlab` named `unpart`. Use the `cd` command to navigate into the `unpart` directory.

:large_blue_diamond: Use this command to estimate the tree using maximum likelihood criterion:
...
:large_blue_diamond: Create a directory named `fullpart` under `iqlab` that is at the same level as your `unpart` directory and navigate into the new `fullpart` directory.

:large_blue_diamond: Enter this command to run IQ-TREE:
...
{% comment %}
233248.0412, worse (233248.0412 - 232864.3261 = 383.7151 units larger)
{% endcomment %}
...
:large_blue_diamond: Create a directory named `locuscomp` (short for "locus comparison") at the same level as `fullpart` and `unpart` and navigate into it.

First, we need to get the two trees we want to compare into the same file. 

:large_blue_diamond: Use the `cat` ("concatenate") command to merge the two files `unpart.treefile` and `fullpart.treefile` into a new file named `turtle.trees`:

    cat ../unpart/unpart.treefile ../fullpart/fullpart.treefile > turtle.trees
    
:large_blue_diamond: Second, run IQ-TREE to generate a file of locus-log-likelihoods:
...
:large_blue_diamond: Delete the first line (`2 29`) of this file and edit the two remaining lines to look like this (except your version will not have any ellipses!):
...
:large_blue_diamond: Now add a command at the end of the file to compute a vector of differences between the log likelihoods for Tree1 and Tree2 for each locus:
...
:large_blue_diamond: Finally, add one more line telling R to create a bar plot showing these 29 differences graphically:
...
:large_blue_diamond: Load the R module so that you can use R to run this file:

    module load r
    
:large_blue_diamond: Now execute your `locuscomp.R` file in R:

    R --no-save < locuscomp.R
    
This should generate a file named `Rplots.pdf`. 

:large_blue_diamond: Pull this file back down to your laptop (there is no easy way to view it on the cluster) using Cyberduck or `scp` and open it.
...
:large_blue_diamond: Create a new directory `final` at the same level as `unpart`, `fullpart`, and `locuscomp` and navigate into it.

:large_blue_diamond: Copy the `turtle.nex` file from the parent directory into your new `final` directory:

    cp ../turtle.nex finalpart.nex
    cp ../turtle.nex finalunpart.nex
    
:large_blue_diamond: Edit the `finalpart.nex` in nano, removing the two lines corresponding to the two paralogous loci.
    
:large_blue_diamond: Edit the `finalunpart.nex` file to remove the two problematic loci and pool sites from all other loci into a single big subset. Your `finalunpart.nex` file should look like this:
...
:large_blue_diamond: Run IQ-TREE to estimate an ML tree for the unpartitioned data:

    iqtree3 -s ../turtle.fa -p finalunpart.nex -B 1000 -nt AUTO --prefix unpart
    
:large_blue_diamond: Run IQ-TREE to estimate an ML tree for the partitioned data:

    iqtree3 -s ../turtle.fa -p finalpart.nex -B 1000 -nt AUTO --prefix fullpart
    
:large_blue_diamond: Finally, ask IQ-TREE to use the partition-finder algorithm to merge loci that are evolving at a rate that is similar enough that they can be modeled using the same relative rate:
...
{% comment %}
------------------------------------------------------------------
              Hypothesis     ufboot             BIC      delta-BIC
------------------------------------------------------------------
unpart                BC        100     219230.8746       +437.475
------------------------------------------------------------------
fullpart              BC        100     219673.2462       +879.847
------------------------------------------------------------------
bestpart              BC        100     218793.3992           +0.0
------------------------------------------------------------------
{% endcomment %}
